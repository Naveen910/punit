...
# --------------------------------------------------
# LIMS CSV Export Function (File-Based Integration)
# and HTTP Client (moved up to avoid usage-before-definition)
# --------------------------------------------------
def export_to_lims_csv(record, lims_folder=r"C:\LIMS_UPLOAD"):
    """
    Export each test record as a standalone CSV file for LIMS pickup.
    """
    try:
        import csv
        from datetime import datetime
...
def export_all_presets():
    """Helper to export preset records P1..P6 to LIMS using globals.
    Keeps behavior identical but centralises the repeated export blocks.
    """
    global test_id, date, time1, P1_FileName, P1_Drop_Temp, P1_Drop_Point
    global P2_FileName, P2_Drop_Temp, P2_Drop_Point
    global P3_FileName, P3_Drop_Temp, P3_Drop_Point
    global P4_FileName, P4_Drop_Temp, P4_Drop_Point
    global P5_FileName, P5_Drop_Temp, P5_Drop_Point
    global P6_FileName, P6_Drop_Temp, P6_Drop_Point
    global Block_float
    ...
    for idx, (fname, dtemp, dpoint) in enumerate(presets, start=1):
        record = {
            "Test_ID": test_id,
            "Date": date,
            "Time": time1,
            ...
            "Drop_Point": round(dpoint) if isinstance(dpoint, (int, float)) else 0,
            "Instrument_ID": "DP-01",
    ...
    def upload_file(self, file_name, data):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not self.login('admin', 'password'):
                    return False
...
if not os.path.isfile(r"D:\Dropping_Point\Imp_datas_dont_delete.CSV"):
    ...
    try:
        ...
        # Variables may not be initialised yet at import-time; skip
        pass
...
while True:
    ...
    if main_add == b'\x08' and touch_add == b'\x24':
        ...
        if Sear_Type == 'File Name':
            a = 3
        elif Sear_Type == 'Drop Temp':
            a = 4
        elif Sear_Type == 'Block Temp':
            a = 5
        elif Sear_Type == 'Drop Point':
            a = 6
        else:
            a = 3
    ...
    if main_add == b'\x08' and touch_add == b'\x52' and day_1 != 0 and mon_1 != 0 and year_1 != 0:
        ...
        ser.write(b'\xaa\x9c' + year_2 + mon_1.to_bytes(1, 'big') + day_1.to_bytes(1, 'big') + hour_1.to_bytes(1, 'big') + min_1.to_bytes(
    ...
    if __name__ == "__main__":
        ...
        client.upload_csv_file("D:\Dropping_Point\DATA.CSV","DATA.CSV")